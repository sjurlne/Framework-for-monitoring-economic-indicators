...
def _multiselect_and_show_table_nor(driver, element_ids, labels, default_select):
    """Takes a Chrome webdriver activated on the particular webpage, selects specified
    variables from a dropdown menu, navigates through different pages, and downloads a
    CSV file.
    ...
    """
    variables = WebDriverWait(driver, 10).until(
        EC.presence_of_element_located((By.ID, element_ids[0]))
    )
    ...
    select_all_years = WebDriverWait(driver, 10).until(
        EC.element_to_be_clickable((By.ID, element_ids[1]))
    )
    driver.execute_script("arguments[0].click();", select_all_years)
    driver.implicitly_wait(10)

    industry_specification = WebDriverWait(driver, 10).until(
        EC.presence_of_element_located((By.ID, element_ids[2]))
    )
    select = Select(industry_specification)
    select.select_by_value("vs__NRNaeringA38NP")
    driver.implicitly_wait(10)

    select_all_industries = WebDriverWait(driver, 10).until(
        EC.element_to_be_clickable((By.ID, element_ids[3]))
    )
    driver.execute_script("arguments[0].click();", select_all_industries)
    driver.implicitly_wait(10)

    submit = WebDriverWait(driver, 10).until(
        EC.element_to_be_clickable((By.ID, element_ids[4]))
    )
    driver.execute_script("arguments[0].click();", submit)
    driver.implicitly_wait(10)

    download = WebDriverWait(driver, 10).until(
        EC.element_to_be_clickable((By.ID, element_ids[6]))
    )
    driver.execute_script("arguments[0].click();", download)
    time.sleep(5)


def _multiselect_and_show_table_den(
    driver, element_ids, deselect, labels, assets, prices, run
):
    """Takes a Selenium WebDriver that navigates to a specific webpage and saves
    specified tables from each side.
    ...
    var = f"var{nr}"
    variables = WebDriverWait(driver, 10).until(
        EC.presence_of_element_located((By.NAME, var))
    )
    select = Select(variables)
    for label in labels:
        select.select_by_visible_text(label)
    ...
    if assets:
        nr += 1
        var = f"var{nr}"
        select_assets = WebDriverWait(driver, 10).until(
            EC.presence_of_element_located((By.NAME, var))
        )
    ...
    sectors = WebDriverWait(driver, 10).until(
        EC.presence_of_element_located((By.NAME, grouping))
    )
    select = Select(sectors)
    select.select_by_visible_text("Select all")
    deselect_sectors = WebDriverWait(driver, 10).until(
        EC.presence_of_element_located((By.NAME, var))
    )
    ...
    if prices:
        nr += 1
        var = f"var{nr}"
        select_price = WebDriverWait(driver, 10).until(
            EC.presence_of_element_located((By.NAME, var))
        )
    ...
    show_table = WebDriverWait(driver, 5).until(
        EC.element_to_be_clickable((By.NAME, element_ids[0]))
    )
    driver.execute_script("arguments[0].click();", show_table)

    driver.implicitly_wait(10)

    pivot = WebDriverWait(driver, 10).until(
        EC.element_to_be_clickable((By.XPATH, element_ids[1]))
    )
    ...
    download = WebDriverWait(driver, 10).until(
        EC.presence_of_element_located((By.NAME, element_ids[2]))
    )
...
def _multiselect_and_show_table_swe(driver, element_ids, sectors, variables, prices):
    """Takes a Chrome webdriver activated on the particular webpage, selects specified
    variables from multiple dropdown menus, clicks on various elements to generate a
    pivot table, and downloads a CSV file.
    ...
    sector_box = WebDriverWait(driver, 10).until(
        EC.presence_of_element_located((By.ID, element_ids[1]))
    )
    ...
    if len(variables) >= 1:
        variable_box = WebDriverWait(driver, 10).until(
            EC.presence_of_element_located((By.ID, element_ids[2]))
        )
    ...
    show_table = WebDriverWait(driver, 5).until(
        EC.element_to_be_clickable((By.ID, element_ids[5]))
    )
    driver.execute_script("arguments[0].click();", show_table)
    driver.implicitly_wait(10)

    pivot_manual = WebDriverWait(driver, 10).until(
        EC.element_to_be_clickable((By.ID, element_ids[6]))
    )
    ...
    if len(variables) > 2:
        upper_box = WebDriverWait(driver, 10).until(
            EC.presence_of_element_located((By.ID, element_ids[7]))
        )
        select = Select(upper_box)
        select.select_by_index(1)

        pivot_manual = WebDriverWait(driver, 10).until(
            EC.element_to_be_clickable((By.ID, element_ids[8]))
        )
        driver.execute_script("arguments[0].click();", pivot_manual)
        driver.implicitly_wait(10)

        lower_box = WebDriverWait(driver, 10).until(
            EC.presence_of_element_located((By.ID, element_ids[9]))
        )
    ...
    pivot_manual = WebDriverWait(driver, 10).until(
        EC.element_to_be_clickable((By.ID, element_ids[10]))
    )
    driver.execute_script("arguments[0].click();", pivot_manual)
    driver.implicitly_wait(10)

    pivot_manual = WebDriverWait(driver, 10).until(
        EC.element_to_be_clickable((By.ID, element_ids[11]))
    )
    driver.execute_script("arguments[0].click();", pivot_manual)
    driver.implicitly_wait(10)

    save_as = WebDriverWait(driver, 10).until(
        EC.element_to_be_clickable((By.ID, element_ids[12]))
    )
    driver.execute_script("arguments[0].click();", save_as)
    driver.implicitly_wait(10)

    excel = WebDriverWait(driver, 10).until(
        EC.element_to_be_clickable((By.XPATH, element_ids[13]))
    )
    driver.execute_script("arguments[0].click();", excel)
    driver.implicitly_wait(10)

    download = WebDriverWait(driver, 10).until(
        EC.element_to_be_clickable((By.ID, element_ids[14]))
    )
    driver.execute_script("arguments[0].click();", download)
    driver.implicitly_wait(10)
    time.sleep(5)
...
def scrape_norway(driver, element_ids, variable_names, default_select, SITES_nor):
    ...
    Args:
        ...
        labels: variable labels that the crawler selects
    ...
    for _ in range(len(SITES_nor)):
        ...
        _multiselect_and_show_table_nor(
            driver, element_ids, variable_names[_], default_select[_]
        )
        driver.implicitly_wait(10)
        time.sleep(5)
...
def scrape_denmark(
    ...
    """Scrapes financial data from multiple webpages on a Danish statistical website
    (statbank.dk) using a Chrome webdriver.
    ...
    for _ in range(len(SITES_den)):
        ...
        _multiselect_and_show_table_den(
            driver,
            element_ids_den,
            default_select_den,
            variable_names_den[_],
            assets_den[_],
            prices_den[_],
            run=_,
        )
...
def scrape_sweden(
    driver, element_ids_swe, sectors_swe, variable_names_swe, prices_swe, SITES_swe
):
    """Scrapes financial data from multiple webpages on the Swedish statistical (scb.se)
    website using a Chrome webdriver.
    ...
    for _ in range(len(SITES_swe)):
        ...
        _multiselect_and_show_table_swe(
            driver, element_ids_swe, sectors_swe, variable_names_swe[_], prices_swe[_]
        )
        time.sleep(5)
        driver.implicitly_wait(10)
...
def remove_old_files(download_dir):
    """Remove old files in the download directory, as selenium ChromeDriver struggles
    with already existing file names.
...
def success(path):
    """Checks whehter the crawler has resulted in a successful donwload for all
    countries."""
