...
def _clean_data_nor(df):
    ...
    after sector and year, and drops rows with missing values.
...
def _clean_data_den(df):
    """Takes raw data from dst.dk as a pandas dataframe, removes empty columns and rows,
    sets column names, sets sector names for all rows, fills missing year values with
    previous year's value, sorts data by sector and year, and drops rows with missing
    values.
    ...
    if len(names) > 1:
        ...
        agg_func = {names[0]: "last", names[1]: "first"}
...
def _fill_col_CF_den(df, col, last_year):
    """Fills missing values in a column of a DataFrame by copying the last non-null
    value.
...
def _cap_form_fix_den(df):
    """Fixes the capital formation data in the given DataFrame for the Denmark dataset
    by filling in missing values from earlier years. This is done by calculating the
    average yearly change for each sector after the first year of data, and using it to
    fill in missing values in earlier years. The resulting DataFrame is sorted by sector
    and year, and missing values after the last year of available data are filled in
    ...
    for sector in sectors:
        ...
        avg_change = np.mean(
            sector_rows[sector_rows["year"] >= first_year][column].diff().fillna(0)
        )
    ...
    df = df.groupby("sector", group_keys=False).apply(
        _fill_col_CF_den, column, last_year
    )
...
def _clean_data_swe(df, col_names):
    """Cleans raw data from Statistics Sweden by removing empty rows and columns,
    setting column names and sector names for all rows, dropping NA rows, and replacing
    '..' values with NaN.
...
def _merge_nor_den(df1, df2, df3, df4=pd.DataFrame([])):
    """Takes three (or four) data frames, and stack them together from the right on
    sector and year.
...
def _merge_swe(df1, df2, df3, df4):
    """Takes four data frames, and stack them together from the right on sector and
    year.
...
def _rename_frame(df):
    """Renames columns in a given DataFrame with a given order to a standardized set of
    names.
...
def clean_and_merge_nor(capital, hours, value_added):
    """Cleans and merges three dataframes containing information on capital, hours, and
...
def replace_sector_names(df, sector_dict):
    """Replaces sector names in a pandas DataFrame with new names defined in a sector
    dictionary.
