...
from final_project_productivity.config import SRC
from final_project_productivity.final import plot_prod
from final_project_productivity.final import plot_sector_data

countries = ["norway", "denmark", "sweden"]
country_names = ["Norway", "Denmark", "Sweden"]
sectors_for_comparison = [
    "Construction",
    "Tranport and storage",
...
for sector in sectors_for_comparison:
    ...
    @pytask.mark.task
    @pytask.mark.depends_on(
        BLD / "python" / "estimates" / "comparing_tables" / f"{sector}.csv"
    )
    @pytask.mark.produces(
        BLD / "python" / "figures" / "sectors" / f"{sector}_compared.png"
    )
    def task_plot_compare(depends_on, produces, sector=sector):
        """Generates a plot comparing the productivity of a given economic sector across
        multiple countries, on the data in the input file specified by the `depends_on`
        argument. The resulting plot is saved in the file specified by the `produces`
        argument.
