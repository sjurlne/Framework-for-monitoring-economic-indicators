...
def plot_prod(
    ...
    """Plots the level of a given production measure over time for the specified number
    of largest sectors in the economy.

    Args:
    - plot_df: A pandas DataFrame in wide format with columns 'year' and columns for each sector's level
    of the production measure.
    - sector_df: A pandas DataFrame with one column of sector names in the same format as the sector names
    used in plot_df.
    ...
    cols = [
        ...
        if col.startswith(starts_with) and any(sector in col for sector in sectors)
    ]
    ...
    fig.update_layout(
        ...
        legend=dict(y=-0.2, borderwidth=0.5),
        hovermode="x unified",
        height=800,
        width=800,
        xaxis=dict(
...
def plot_sector_data(data, sector, country_names, layout, productivity="level_TFP"):
    """Create a line plot of a given sector's data over time for different countries
    using Plotly.
    ...
    Args:
        ...
        productivity (str, optional): The type of productivity data to plot. Defaults
        to "level_TFP".
